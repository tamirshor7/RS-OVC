...
class ODVGDataset(VisionDataset):
    ...
    def _load_metas(self, anno):
        with open(anno, "r") as f:
            #this pre-processing is needed because some of our curated datasets are not single object per-image datasets.
            self.metas = []
            for line in f:
                cur_meta = json.loads(line)
                label_sep_cur = []
                filename,height, width = cur_meta['filename'], cur_meta['height'], cur_meta['width']
                instances = cur_meta['detection']['instances']
                instance_labels = set([x['label'] for x in instances])
                
                #assert len(instance_labels)==1
                for label in instance_labels:
                    label_sep_cur.append({'filename':filename,'height':height,'width':width,\
                        'detection':{'instances':[x for x in instances if x['label']==label]}})
                self.metas += label_sep_cur
    ...
    def __getitem__(self, index: int):
        ...
        if "exemplars" not in meta: #DIOR case
            meta["exemplars"] = [x['bbox'] for x in meta['detection']['instances'] if x['area']!=4]
...
def make_coco_transforms(image_set, fix_size=False, strong_aug=False, args=None):
    ...
    #         57.12,
    #         57.375,
    #     ]) #DIOR stats - using ImageNet stats worked better
