...
class DINOv3BackboneWrapper(nn.Module):
    """
    Wraps a pre-trained DINOv3 model to function as a backbone in CountGD.
    Adapts NestedTensor inputs (Swin-normalized) to DINOv3 inputs, 
    and reshapes outputs to 4D feature maps.
    """
    ...
    def __init__(self, model_name: str):
        ...
        # We use the processor for resizing/normalization consistency
        self.processor = AutoImageProcessor.from_pretrained(model_name)
        self.model = AutoModel.from_pretrained(model_name)
            
        # Extract configuration for reshaping
        self.hidden_dim = self.model.config.hidden_size
        self.patch_size = self.model.config.patch_size
        
        # Standard ImageNet mean/std (used by Swin transforms) for denormalization
    ...
    def forward(self, tensor_list: NestedTensor) -> Dict[int, NestedTensor]:
        """
        Args:
            tensor_list: NestedTensor from Swin backbone (normalized with ImageNet stats).
                         Tensors shape: [B, 3, H, W]
        """
        images = tensor_list.tensors
        device = images.device
        B = images.shape[0]

        # 1. Denormalize: Revert Swin normalization to get approx [0, 1] range
        #    DINOv3 processor expects raw-like pixel values.
        images_denorm = images * self.swin_std + self.swin_mean
        
        # 2. Process for DINOv3
        #    The processor handles resizing to the ViT's expected grid (e.g., 224x224 or 518x518)
        #    and applies DINOv3-specific normalization.
        #    We use the shortest_edge strategy or fixed size based on config.
        
        target_size = {"shortest_edge": 224} # Default fallback
        if hasattr(self.processor, "size"):
             target_size = self.processor.size

        # AutoImageProcessor can handle batches of tensors if configured correctly,
        # but inputs needs to be consistent. 
        inputs = self.processor(
            images=images_denorm, 
            return_tensors="pt",
            do_resize=True,
            size=target_size,
            do_rescale=False,   # Input is already float [0, 1]
            do_normalize=True   # Apply DINOv3 mean/std
        ).to(device)
        ...
        # Handle Tokens: CLS + Registers + Patches
        # DINOv3 (and v2) often have registers. 
        # Typical structure: [CLS, Reg1, Reg2, ..., RegN, Patch1, Patch2, ...]
        # We need to calculate how many non-patch tokens there are.
        
        # Calculate expected number of patches based on input size
        # inputs['pixel_values'] has shape [B, 3, H_in, W_in]
        H_in, W_in = inputs['pixel_values'].shape[-2:]
        H_feat = H_in // self.patch_size
        W_feat = W_in // self.patch_size
        num_patches = H_feat * W_feat
        
        # The sequence length from model
        L_total = sequence_output.shape[1]
        
        # The number of special tokens (CLS + registers) is the difference
        num_special_tokens = L_total - num_patches
        
        # Keep only the patch tokens (assume special tokens are at the beginning)
        if num_special_tokens > 0:
            sequence_output = sequence_output[:, num_special_tokens:, :]

        # Reshape: [B, num_patches, D] -> [B, H, W, D] -> [B, D, H, W]
        feature_4d = sequence_output.view(B, H_feat, W_feat, self.hidden_dim).permute(0, 3, 1, 2)
        
        # 5. Handle Mask Interpolation
        #    We must adapt the original mask (original size) to the DINOv3 feature size.
        m = tensor_list.mask # [B, H_orig, W_orig]
        if m is not None:
            m_float = m.float().unsqueeze(1) # [B, 1, H, W]
            # Nearest neighbor interpolation for boolean masks
            mask_feat = F.interpolate(m_float, size=(H_feat, W_feat), mode='nearest').to(torch.bool)
            mask_feat = mask_feat.squeeze(1)
        else:
            mask_feat = torch.zeros((B, H_feat, W_feat), dtype=torch.bool, device=device)

        # 6. Return formatted dictionary (Key 0 for single scale)
        return {0: NestedTensor(feature_4d, mask_feat)}

def build_rsfm_backbone():
    model_name = "facebook/dinov3-vitl16-pretrain-sat493m" #dinov3 pretrained on RS data
    return DINOv3BackboneWrapper(model_name)
...
class ViTCrossAttention(nn.Module):
    """
    Standard multi-head self/cross attention block (as a placeholder for simplicity).
    For full performance, this would be a more complex Transformer-style attention block.
    Q = Swin-T features, K/V = Projected ViT features.
    """
    def __init__(self, d_model, nhead=8):
        super().__init__()
        # Use a standard PyTorch MultiheadAttention for the cross-attention
        self.attn = nn.MultiheadAttention(d_model, nhead, dropout=0.1, batch_first=True)
        self.norm = nn.LayerNorm(d_model)
        self.d_model = d_model

    def forward(self, q, k, v):
        # q, k, v are of shape [B, H*W, C]
        # output is attn_output [B, H*W, C]
        attn_output, _ = self.attn(q, k, v)
        return self.norm(q + attn_output) # Add & Norm with Residual Connection

class ViTFeatureFusion(nn.Module):
    """
    Handles fusion for one Swin-T feature scale (l).
    Projects ViT channels, performs spatial scaling, and applies Cross-Attention.
    """
    def __init__(self, vit_in_dim, swin_out_dim):
        super().__init__()
        
        # 1. ViT Projection: From ViT's 1024 channels to Swin-T's scale-specific channel (256, 512, or 1024)
        self.vit_proj = nn.Conv2d(vit_in_dim, swin_out_dim, kernel_size=1)
        
        
        # 2. Cross-Attention: Operates on swin_out_dim
        self.cross_attn = ViTCrossAttention(d_model=swin_out_dim, nhead=8)
        

    def forward(self, swin_feat, vit_feat):
        bs, swin_c, swin_h, swin_w = swin_feat.shape
        
        # ViT Projection and Scaling 
        
        # a. project ViT channels: [B, 1024, 49, 48] -> [B, swin_c, 49, 48]
        projected_vit_feat = self.vit_proj(vit_feat)
        
        # b. scale ViT spatially to match Swin-T scale: [B, swin_c, 49, 48] -> [B, swin_c, swin_h, swin_w]
        scaled_vit_feat = F.interpolate(
            projected_vit_feat, 
            size=(swin_h, swin_w), 
            mode='bilinear', 
            align_corners=False # Set to False for non-feature map interpolation
        )
        
        #  Cross-Attention Fusion
        
        # reshape for Attention: [B, C, H, W] -> [B, H*W, C]
        swin_tokens = swin_feat.flatten(2).transpose(1, 2)  # Q: [B, H*W, C]
        vit_tokens = scaled_vit_feat.flatten(2).transpose(1, 2) # K/V: [B, H'*W', C]
        
        # Perform Cross-Attention: Q=Swin-T, K/V=ViT
        fused_tokens = self.cross_attn(
            q=swin_tokens, k=vit_tokens, v=vit_tokens
        )
        
        # reshape back to feature map: [B, H*W, C] -> [B, C, H, W]
        fused_feat = fused_tokens.transpose(1, 2).reshape(bs, swin_c, swin_h, swin_w)
        
        # output (residual operation)
        return swin_feat + fused_feat
...
class GroundingDINO(nn.Module):
    ...
    def __init__(
        ...
        self.rsfm = build_rsfm_backbone()
        self.vit_fusion_blocks = nn.ModuleList([
                    ViTFeatureFusion(1024, swin_out_dim)
                        for swin_out_dim in [256,512,1024]
                    ])
    ...
    def forward(
        ...
        **kw,
        ...
        if not cropped:
            ...
            vit_features = self.rsfm(samples)[0]
            #fused_features = []
            for i, swin_feat_tensor in enumerate([f.tensors for f in features]): # Get tensors from NestedTensor
                # Fusing the Swin-T feature tensor with the ViT feature tensor
                fused_tensor = self.vit_fusion_blocks[i](swin_feat_tensor, vit_features.tensors)
                #fused_features.append(fused_tensor)
                features[i].tensors = fused_tensor
